...
## Synthesis
- [[system-architecture]] -- 系统架构定义软件系统的高层结构，涉及组件边界、通信模式、数据流和部署拓扑等决策，需要在性能、可扩展性、可维护性、安全性和成本之间权衡
- [[event-sourcing]] -- 事件溯源将状态变更存储为不可变的事件序列而非当前状态快照，提供完整审计轨迹并支持时间查询，常与CQRS配合使用
- [[cqrs]] -- CQRS（命令查询职责分离）将读写操作分离为不同模型，命令修改状态不返回数据，查询返回数据不修改状态，允许独立优化
- [[repository-pattern]] -- 仓储模式将数据访问逻辑抽象在接口后使数据源可互换，便于测试时替换为内存实现或模拟对象
- [[decorator-pattern]] -- 装饰器模式动态地为对象附加额外职责，是子类化的灵活替代方案，Python装饰器是语言层面的实现，中间件管道也应用了该概念
- [[strategy-pattern]] -- 策略模式定义一族可互换的算法并分别封装，使算法可独立于使用它的客户端变化，在现代语言中通过函数对象或Lambda可实现更轻量的形式
- [[observer-pattern]] -- 观察者模式定义一对多依赖关系，当被观察对象状态变化时自动通知所有依赖者，是事件驱动架构、响应式编程和发布-订阅系统的基础
- [[factory-pattern]] -- 工厂方法模式将对象创建逻辑封装在工厂方法中，使客户端代码与具体类解耦，特别适用于插件架构和跨平台抽象
- [[singleton-pattern]] -- 单例模式确保一个类只有一个实例并提供全局访问点，但因隐藏依赖和妨碍测试而逐渐被依赖注入容器等现代替代方案取代
- [[design-patterns]] -- 设计模式是软件设计中常见问题的可复用解决方案，1994年由Gang of Four首次编目，为架构决策提供共享的工程词汇
- [[pipeline-as-code]] -- 管道即代码是在版本控制配置文件中定义CI/CD管道的实践，使管道变更可审查、可版本化、可复现，支持模板化和跨仓库共享
- [[continuous-delivery]] -- 持续交付在CI基础上自动将代码构建为不可变的、版本化的部署制品并存入制品仓库，持续部署则进一步将每项通过所有检查的变更自动部署到生产环境
- [[trunk-based-development]] -- 主干开发是一种短生命周期分支策略，开发者频繁合并到主分支，配合功能开关使代码库始终处于可发布状态
- [[continuous-integration]] -- 持续集成是自动构建和测试每次代码提交的实践，包含签出、构建、测试、报告等管道阶段，以及依赖管理和构建性能优化等关键关注点
- [[mob-programming]] -- 集体编程将结对编程扩展到整个团队共同解决同一问题，以个人灵活性换取更高的 bus factor 和实时知识共享。
- [[pair-programming]] -- 结对编程是一种实时代码审查机制，两名开发者共用工作站，一人写代码一人实时审查，消除异步审查延迟。
- [[code-review-culture]] -- 健康的审查文化要求聚焦代码而非作者、作者以开放心态接受建议、分歧时使用 RFC 文档前置讨论设计决策。
- [[pre-commit-hooks]] -- Pre-commit 钩子在代码到达审查前本地运行自动化检查，减少琐碎修正的来回沟通，提升审查效率。
- [[automated-code-review-tools]] -- 自动化工具（linter、格式化工具、类型检查器、静态分析器）处理机械性问题，让人工审查者聚焦于设计与逻辑。
- [[code-review-checklist]] -- 代码审查清单帮助审查者系统性地检查常见问题，包括测试覆盖、错误处理、安全顾虑、新依赖、文档更新和跨团队影响等维度。
- [[conventional-comments]] -- Conventional Comments 是一种结构化的代码审查反馈格式，通过标准标签（如 nit:、suggestion:）统一表达反馈的意图和严重程度。
- [[blocking-issues-vs-suggestions]] -- 审查者应区分阻塞问题（正确性缺陷、安全漏洞、架构违规）和建议（风格偏好、替代方案），使用 nit: 或 suggestion: 等标签标示严重程度。
- [[review-turnaround-time]] -- 最佳实践建议在一个工作日内完成审查，若无法快速完成需先给作者初始确认；研究表明每次审查200-400行代码效果最佳。
- [[code-review]] -- 代码审查是对源代码变更在合并前进行的系统性检查，核心目标包括缺陷检测、知识共享、一致性保持和指导初级开发者。
